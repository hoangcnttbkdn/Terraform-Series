locals {
  ...
  need_ami_id          = local.features_require_ami && length(var.ami_image_id) == 0 ? true : false
  ...
  get_cluster_data = (local.need_cluster_kubernetes_version || local.need_bootstrap || local.need_ssh_access_sg || length(var.associated_security_group_ids) > 0)
  ...
  autoscaler_enabled_tags = {
    ...
    "k8s.io/cluster-autoscaler/enabled"             = var.cluster_autoscaler_enabled
  }
