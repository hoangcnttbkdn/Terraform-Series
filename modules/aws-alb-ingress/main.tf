...
resource "aws_iam_policy" "this" {
  name        = "${local.name}-${var.irsa_role_name_prefix}-policy"
  path        = "/"
...
data "aws_iam_policy_document" "this_assume" {
  statement {
    ...
    condition {
      ...
      values = [
        "system:serviceaccount:${var.namespace}:${local.name}-alb-controller",
      ]
    }
    effect = "Allow"
  }
...
resource "aws_iam_role" "this" {
  name               = "${local.name}-${var.irsa_role_name_prefix}"
...
resource "kubernetes_service_account" "service_account" {
  metadata {
    ...
    labels = {
      "app.kubernetes.io/name"      = "${local.name}-alb-controller"
